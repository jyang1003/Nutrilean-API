	...
	Promise.resolve(req.body.credentials)
		...
		.then(() => {
			...
			return {
				...
				hashedPassword: req.body.credentials.password,
			}
		})
...
router.post('/sign-in', (req, res, next) => {
	...
	User.findOne({ email: req.body.credentials.email })
		.then((record) => {
			...
			return (user.hashedPassword == req.body.credentials.password)
		})
...
router.patch('/change-password', requireToken, (req, res, next) => {
	...
	User.findById(req.user._id)
		...
		.then((record) => {
			user = record
			console.log(record)
			if (req.body.passwords.old, user.hashedPassword){
				if (!req.body.passwords.new) {
					throw new BadParamsError()
				}
			}
		})
		.then((hash) => {
			...
			user.hashedPassword = req.body.passwords.new
			return user.save()
		})
